...
* [Git изнутри](#git-изнутри)
> * [Внутреннее строение](#внутреннее-строение)
> > * [Blob](#blob)
> > * [Tree](#tree)
> > * [Commit](#commit)
> > * [Символьные имена объектов](#символьные-имена-объектов)
...
Перемещение веток.
Для перемещения ветки требуется перейти в ветку которую нужно переместить,
а потом указать ветку в которую перемещаем.

    git checkout <rebased-branch-name>
    git rebase <branch-name>

Так же можно переместить одну ветку в другую,
...
Для перемещения требуется перейти в ветку feature_1.
...
Если произошли конфикты решить их.
...
Если требуется переместить указать master на feature_1.
...
но feature_2 не зависит ни от каких изменений в feature_1
и может быть просто ответвлением от master.
...
Для того чтобы объединить коммиты нужно заменить `pick` на `squash`. 
...
Объекты бывают трёх типов - blob, tree, commit.
...
*Примеры:*
Дерево содержит в себе несколько blob обхектов.
