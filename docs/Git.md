...
Стандартные настроки, которые можно задать,
опция `--global` позволяет настроить для всех проектов информацию.
Если требуется для одного проекта другие настройки,
нужно зайти в проект и сделать там настройки уже без опции `--global`.
