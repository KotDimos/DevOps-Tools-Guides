...
## Отображение имени пользователя в текущей оболочке
...
LVM (Logical Volume Manager) – подсистема операционных систем Linux,
...
* `-l` - посмотреть файлы в архиве.
* `-d <dir>` - указание по какому пути требуется расспаковать.
