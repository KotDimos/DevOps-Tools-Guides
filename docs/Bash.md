...
При использовании одинарных качвычек, каждый символ будет обрабатываться буквально.
При двойных кавычках, будет происходить подстановка.
...
-lt, -gt являются аргументами.
Поэтому знаки `>` и `<` странно себя ведут внутри одинарных квадратных скобок. 
Интерпритатор пытается перенаправить input или output внутри команды.
...
Их можно использовать даже без if, как пример инкрементирование счётчика.
...
Как пример, если нужно обработать файлы директории то можно передать команду `$(ls dir)`,
...
Кэйс с `*` отвечает любые другие возможные значения.
...
* `[:xdigit:]` - набор шестнадцатиричных цифр. Эквивалентно выражению [0-9A-Fa-f].
